...
# Time-based greeting
"""
   this  Generates a time-based greeting (morning, afternoon, evening). and 
    :return: A string greeting based on the current hour.
    """
...
# Add this function after generate_agent_name()
def get_user_name():
    name_window = tk.Tk()
    ...
    tk.Label(name_window, text="Please enter your name:", 
             font=("Helvetica", 12), bg="#f5f5f5").pack(pady=10)
    
    name_entry = tk.Entry(name_window, textvariable=user_name, font=("Arial", 12))
    name_entry.pack(pady=10)
    name_entry.focus()  # Auto-focus the name entry
    
    submit_btn = tk.Button(name_window, text="Submit", command=submit_name,
              bg="#4CAF50", fg="#fff", font=("Times new roman", 10, "bold"))
    submit_btn.pack()
    ...
    return user_name.get().strip() or "Guest"
...
# Tkinter GUI Setup
user_name = get_user_name()  # Get username first
...
tk.Label(root, text=f"Hello {user_name}! I'm {agent_name}. How can I assist you today?",
...
chat_window.insert(tk.END, f"{agent_name}: Hello {user_name}! How can I help?\n")
